...
public final class InGameInputHandler extends InputHandler {
	 ...
	 * @return The integer value of the attribute, or Integer.MIN_VALUE on failure.
	 */
	...
	/**
	 * Sometimes units appear which the client does not know about, and are passed
	 * in as the children of the parent element. Worse, if their location is a Unit,
	 * that unit has to be passed in too. Pull a unit out of the children by id.
	 *
	 * @param game
	 ...
	 *
	 * @param element
	 *            The element (root element in a DOM-parsed XML tree) that holds all
	 *            the information.
	 * @return Null.
	 */
	...
	/**
	 * Handle an "animateAttack"-message. This only performs animation, if required.
	 * It does not actually perform any attacks.
	 *
	 * @param element
	 *            An element (root element in a DOM-parsed XML tree) that holds
	 *            attributes for the old and new tiles and an element for the unit
	 *            that is moving (which are used solely to operate the animation).
	 * @return Null.
	 */
	...
	/**
	 * Handle an "animateMove"-message. This only performs animation, if required.
	 * It does not actually change unit positions, which happens in an "update".
	 *
	 * @param element
	 *            An element (root element in a DOM-parsed XML tree) that holds
	 *            attributes for the old and new tiles and an element for the unit
	 ...
	 * that caused a popup (for example, a native demand or diplomacy proposal) has
	 * not been answered quickly enough and that the offering player has assumed
	 * this player has refused-by-inaction, and therefore, the popup needs to be
	 * closed.
	 *
	 * @return Null.
	 */
	 ...
	 * proposal, then ask the user about it and return the response with appropriate
	 * response set.
	 *
	 * @param element
	 *            The element (root element in a DOM-parsed XML tree) containing a
	 *            "diplomacy"-message.
	...
	/**
	 * Disposes of the <code>Unit</code>s which are the children of this Element.
	 *
	 * @param element
	 *            The element (root element in a DOM-parsed XML tree) that holds all
	 *            the information.
	 ...
	 *            The element (root element in a DOM-parsed XML tree) that holds all
	 *            the information.
	 * @return An <code>IndianDemand</code> message containing the response, or null
	 *         on error.
	 */
