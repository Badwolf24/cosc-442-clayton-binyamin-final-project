...
public final class InGameController implements NetworkConstants {
	...
	/**
	 * Display the colony panel for a colony, and select the unit that just arrived
	 * there if it is a carrier.
	 *
	 * @param colony
	...
	/**
	 * Convenience function to find an adjacent settlement. Intended to be called in
	 * contexts where we are expecting a settlement to be there, such as when
	 ...
	 * Intended to be called in contexts where we are expecting a settlement or unit
	 * to be there, such as when handling a particular move type.
	 *
	 * @param tile
	 *            The <code>Tile</code> to start at.
	 * @param direction
	 *            The <code>Direction</code> to step.
	 * @return The name of the nation controlling a settlement on the adjacent tile
	 *         if any.
	 */
	 ...
	 *
	 * This is unusual for an ask* routine in that it uses a *Was structure, but it
	 * is needed to extract the unit.
	 *
	 * @param europe
	...
	/**
	 * Select all the units to emigrate from Europe. If they are all the same they
	 * can be picked automatically, but otherwise use the emigration dialog. Only to
	 * be called if the player is allowed to select the unit type (i.e. FoY or has
	 * Brewster).
	 *
	 * The server contains the count of available FoY-units, and maintains the
	 * immigration/immigrationRequired amounts, so this routine will fail harmlessly
	...
	private void emigration(Player player, int n, boolean fountainOfYouth) {
		...
		for (; n > 0 || player.checkEmigrate(); n--) {
			if (!allSame(europe.getRecruitables())) {
				final int nf = n;
				gui.showEmigrationDialog(player, fountainOfYouth, (Integer value) -> { // Value is a valid slot
	...
	/**
	 * Creates at least one autosave game file of the currently played game in the
	 * autosave directory. Does nothing if there is no game running.
	 */
	...
	/**
	 * See if messages with a given key were ignored last turn. If so, continue to
	 * ignore them.
	 *
	 * @param key
	 *            The key to check.
	 ...
	 * unit just died).
	 *
	 * Displays any new <code>ModelMessage</code>s with {@link #nextModelMessage}.
	 *
	 * @param tile
	 *            The <code>Tile</code> to select if no new unit can be made active.
	...
	/**
	 * Confirm attack or demand a tribute from a settlement, following an attacking
	 * move.
	 *
	 * @param unit
	 *            The <code>Unit</code> to perform the attack.
	...
	/**
	 * Check the carrier for passengers to disembark, possibly snatching a useful
	 * result from the jaws of a MOVE_NO_ACCESS_LAND failure.
	 *
	 * @param unit
	...
	/**
	 * Embarks the specified unit onto a carrier in a specified direction following
	 * a move of MoveType.EMBARK.
	 *
	 * @param unit
	...
	/**
	 * Moves a unit onto the "high seas" in a specified direction following a move
	 * of MoveType.MOVE_HIGH_SEAS. This may result in a move to Europe, no move, or
	 * an ordinary move.
	 *
	 * @param unit
	...
	/**
	 * Move a free colonist to a native settlement to learn a skill following a move
	 * of MoveType.ENTER_INDIAN_SETTLEMENT_WITH_FREE_COLONIST. The colonist does not
	 * physically get into the village, it will just stay where it is and gain the
	 * skill.
	 *
	 * @param unit
	...
	/**
	 * Move to a foreign colony and either attack, negotiate with the foreign power
	 * or spy on them. Follows a move of MoveType.ENTER_FOREIGN_COLONY_WITH_SCOUT.
	 *
	...
	/**
	 * Move a scout into an Indian settlement to speak with the chief, or demand a
	 * tribute following a move of MoveType.ENTER_INDIAN_SETTLEMENT_WITH_SCOUT. The
	 * scout does not physically get into the village, it will just stay where it
	 * is.
	 *
	 * @param unit
	 ...
	 * (Deliberate use of Settlement rather than IndianSettlement throughout these
	 * routines as some unification with colony trading is anticipated, and the
	 * native AI already uses the same DeliverGiftMessage to deliver gifts to
	 * Colonies).
	 *
	 * @param unit
	 ...
	 * @param messages
	 *            An optional list in which to retain any <code>ModelMessage</code>s
	 *            generated.
	 ...
	 * @param toLoad
	 *            The amount of goods the unit should load according to the trade
	 *            route orders.
	 ...
	 *
	 * Normally just state that a certain amount of goods was unloaded. Make special
	 * mention if the actual unloaded amount was short (unloaded &lt; amount), or an
	 * overflow is happening (amount &gt; atStop) in which case distinguish dumping
	 * (amount == toUnload) from retaining on board).
	 *
	 * @param unit
	 ...
	 * @param toUnload
	 *            The amount of goods that should be unloaded according to the trade
	 *            route orders.
	...
	/**
	 * Check if a unit is a treasure train, and if it should be cashed in. Transfers
	 * the gold carried by this unit to the {@link Player owner}.
	 *
	 ...
	 * @return A counter agreement, a rejected agreement, or null if the original
	 *         agreement was already decided.
	 */
	 ...
	 *
	 * @param high
	 *            A <code>Boolean</code> whose values indicates whether a new high
	 *            score has been achieved, or no information if null.
	...
	public boolean firstContact(Player player, Player other, Tile tile, boolean result) {
		//
		if (player.equals(null) || player.equals(null) || player.equals(other) || tile.equals(null))
			return false;
	...
	/**
	 * Moves the active unit in a specified direction. This may result in an attack,
	 * move... action.
	 *
	 ...
	 *
	 * Called from CargoPanel, EuropePanel.MarketPanel, EuropePanel.unloadAction,
	 * QuickActionMenu
	 *
	 * @param type
	...
	/**
	 * Buys the remaining hammers and tools for the {@link Building} currently being
	 * built in the given <code>Colony</code>.
	 *
	 ...
	 * @param recover
	 *            A <code>Runnable</code> to restore the normal player view of the
	 *            tile when the spying colony panel is closed.
	 */
	...
	/**
	 * Unload cargo. If the unit carrying the cargo is not in a harbour, or if the
	 * given boolean is true, the goods will be dumped.
	 *
