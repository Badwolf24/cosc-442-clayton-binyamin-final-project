...
public final class MapViewer {
	...
	private static final int UNIT_OFFSET = 20, OTHER_UNITS_OFFSET_X = -5, // Relative to the state indicator.
	...
	/**
	 * Converts the given screen coordinates to Map coordinates. It checks to see to
	 * which Tile the given pixel 'belongs'.
	 *
	 * @param x
	...
	/**
	 * Calculate the bounds of the rectangle containing a Tile on the screen, and
	 * return it. If the Tile is not on-screen a maximal rectangle is returned. The
	 * bounds includes a one-tile padding area above the Tile, to include the space
	 ...
	 * @return The position of the given <code>Tile</code>, or <code>null</code> if
	 *         the <code>Tile</code> is not drawn on the mapboard.
	 */
	...
	/**
	 * Checks if the Tile/Units at the given coordinates are displayed on the screen
	 * (or, if the map is already displayed and the focus has been changed, whether
	 * they will be displayed on the screen the next time it'll be redrawn).
	 *
	 * @param tileToCheck
	 ...
	 *
	 * @param focus
	 *            The <code>Position</code> of the center tile of the displayed map.
	 * @see #getFocus
	 */
	...
	/**
	 * Sets the focus of the map but offset to the left or right so that the focus
	 * position can still be visible when a popup is raised. If successful, the
	 * supplied position will either be at the center of the left or right half of
	 * the map.
	 *
	 * @param tile
	 *            The <code>Tile</code> to display.
	 * @return Positive if the focus is on the right hand side, negative if on the
	...
	private void positionMap(Tile pos) {
		...
		/*
		 * PART 1 ====== Calculate: bottomRow, topRow, bottomRowY, topRowY This will
		 * tell us which rows need to be drawn on the screen (from bottomRow until and
		 * including topRow). bottomRowY will tell us at which height the bottom row
		 * needs to be drawn.
		 */
		alignedTop = false;
		alignedBottom = false;
		...
		/*
		 * PART 2 ====== Calculate: leftColumn, rightColumn, leftColumnX This will tell
		 * us which columns need to be drawn on the screen (from leftColumn until and
		 * including rightColumn). leftColumnX will tell us at which x-coordinate the
		 * left column needs to be drawn (this is for the Tiles where y&1 == 0; the
		 * others should be halfWidth more to the right).
		 */
	...
	/**
	 * Returns the amount of columns that are to the left of the Tile with the given
	 * y-coordinate.
	 *
	 * @param y
	 *            The y-coordinate of the Tile in question.
	 * @return The amount of columns that are to the left of the Tile with the given
	 *         y-coordinate.
	 */
	 ...
	 *
	 * <ol>
	 * <li>If the tile contains a unit that can become active, then that unit will
	 * be set as the active unit.
	 * <li>If not, the <code>selectedTile</code> will become the map focus.
	 * </ol>
	 *
	 * If a unit is active and is located on the selected tile, then nothing (except
	 * perhaps a map reposition) will happen.
	 *
	 * @param newTile
	 ...
	 *
	 * @return The path that should be drawn on the map or <code>null</code> if no
	 ...
	 *
	 * @param gotoPath
	 *            The path that should be drawn on the map or <code>null</code> if
	...
	void displayMap(Graphics2D g) {
		...
		if (!game.isInRevengeMode()) {
			...
			/*
			 * Add extra rows and colums, as the dark halo is huge to enable a very slow
			 * fade into transparency, see BR#2580
			 */
		...
		if (colonyLabels != ClientOptions.COLONY_LABELS_NONE) {
			...
			/*
			 * For settlement names and territorial borders 1 extra row needs to be drawn in
			 * north to prevent missing parts on partial redraws, as they can reach below
			 * their tiles, see BR#2580
			 */
			map.forSubMap(x0, y0 - 1, lastColumn - firstColumn + 1, lastRow - firstRow + 1 + 1, (Tile tile) -> {
	...
	/**
	 * Creates an BufferedImage that shows the given text centred on a translucent
	 * rounded rectangle with the given color.
	 *
	 * @param g
	...
	/**
	 * Draws the borders of a territory on the given Tile. The territory is either a
	 * country or a region.
	 *
	 * @param g
	 *            a <code>Graphics2D</code>
