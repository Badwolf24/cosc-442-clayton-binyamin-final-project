...
public class ScaleMapAction extends FreeColAction {
	...
	/**
	 * Scales the current map into the specified size. The current map is given by
	...
	private void scaleMapTo(final int width, final int height) {
		/*
		 * This implementation uses a simple linear scaling, and the isometric shape is
		 * not taken into account.
		 *
		 * FIXME: Find a better method for choosing a group of adjacent tiles. This
		 * group can then be merged into a common tile by using the average value (for
		...
		/*
		 * Commented because it doesn't appear to do anything valuable // Update river
		 * directions for (Tile t : map.getAllTiles()) {
