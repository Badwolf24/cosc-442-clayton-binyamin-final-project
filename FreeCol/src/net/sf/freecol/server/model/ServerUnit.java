...
public class ServerUnit extends Unit implements ServerModelObject {
	 ...
	 *
	 * -vis: Visibility issues depending on location. -til: Changes appearance if
	 * unit goes into a colony.
	 *
	 * @param game
			...
			if (attrition > getType().getMaximumAttrition()) {
				cs.addMessage(See.only(owner),
						new ModelMessage(ModelMessage.MessageType.UNIT_LOST, "model.unit.attrition", this)
								...
								.addStringTemplate("%location%", loc.getLocationLabelFor(owner)));
		...
		if (!isDamaged()) {
			...
			cs.addMessage(See.only(owner),
					new ModelMessage("model.unit.unitRepaired", this, (FreeColGameObject) loc)
							.addStringTemplate("%unit%", getLabel())
							.addStringTemplate("%repairLocation%", loc.getLocationLabelFor(owner)));
			setState(UnitState.ACTIVE);
		}
	...
	/**
	 * If a unit moves, check if an opposing naval unit slows it down. Note that the
	 * unit moves are reduced here.
	 *
	 * @param newTile
	...
	private boolean csExploreLostCityRumour(Random random, ChangeSet cs) {
		...
		case COLONIST:
			...
			newUnit = new ServerUnit(game, tile, serverPlayer, unitType);// -vis: safe, scout on tile
			cs.addMessage(See.only(serverPlayer),
	...
	/**
	 * Collects the tiles surrounding this unit that the player can not currently
	 * see, but now should as a result of a move.
	 *
	 * @param tile
	...
	public void csMove(Tile newTile, Random random, ChangeSet cs) {
		...
		List<Tile> oldTiles = (oldLocation.getTile().equals(null)) ? Collections.<Tile>emptyList()
